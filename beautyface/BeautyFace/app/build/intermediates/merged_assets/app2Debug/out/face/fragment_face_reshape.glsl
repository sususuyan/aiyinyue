...
vec2 faceLift(vec2 currentCoordinate, float faceLength)
{
    ...
    destPoint = currentPoint + (cartesianPoints[23] - currentPoint) * faceLiftScale;
...
void main()
{
    ...
    if (enableReshape == 0 || (cartesianPoints[70].x / float(textureWidth) <= 0.03)
        || (cartesianPoints[70].y / float(textureHeight)) <= 0.03) {
